...
def log_usage(code, user_email):
    ...
    try:
        with database.get_db_session() as db:
            log_entry = database.PromoLog(code=code, user_email=user_email, used_at=datetime.utcnow())
            db.add(log_entry)
...
def create_code(code, max_uses=1):
    ...
    try:
        with database.get_db_session() as db:
            ...
            if existing: return False, f"Code '{code}' already exists."
            
            new_promo = database.PromoCode(code=code, max_uses=max_uses, active=True, created_at=datetime.utcnow())
            db.add(new_promo)
