...
def inject_legacy_accessibility_css():
    st.markdown("""
        ...
        .stTabs [data-baseweb="tab"] {
            height: 70px; background-color: #F0F2F6; border-radius: 10px 10px 0px 0px;
            border: 3px solid #9CA3AF; margin-right: 5px; color: #374151;
        }
        ...
        .font-preview-box { padding: 20px; border: 1px solid #ddd; border-radius: 10px; background-color: #f9f9f9; text-align: center; margin-bottom: 20px;}
        ...
        .instruction-box { background-color: #FEF3C7; border-left: 10px solid #F59E0B; padding: 20px; margin-bottom: 25px; font-size: 20px; font-weight: 500; color: #000; }
        </style>
...
def initialize_legacy_state():
    defaults = {
        "legacy_sender": {}, "legacy_recipient": {}, "legacy_text": "",
        "legacy_font": "Caveat", "current_legacy_draft_id": None, "last_legacy_hash": None,
        "paid_success": False
    }
...
def render_legacy_page():
    ...
    if c_save.button("💾 Save Progress", use_container_width=True): 
        _save_legacy_draft()
    ...
    with t_type:
        txt = st.text_area("Body", value=st.session_state.get("legacy_text", ""), height=600, label_visibility="collapsed")
    ...
    with t_rec:
        ...
        if audio and ai_engine:
            # FIX #3 & #7: Reliable Hashing
            audio_bytes = audio.getvalue()
            audio_hash = hashlib.md5(audio_bytes).hexdigest()
            
            if audio_hash != st.session_state.get("last_legacy_hash"):
    ...
    with c_c:
        st.info("Total: $15.99 (Certified Mail)")
