...
def render_splash_page():
    ...
    """ + 
    get_img_tag(STRIPE_SVG, 28) +
    get_img_tag(TWILIO_SVG, 32) +
    get_img_tag(OPENAI_SVG, 28) +
    get_img_tag(SUPABASE_SVG, 28) +
    get_img_tag(USPS_SVG, 32) +
    ...
    with col_sec2:
        if st.button("📮 Go to Letter Store", use_container_width=True):
            if st.session_state.get("authenticated"):
                ...
                # FIX: Redirect unauthenticated users to LOGIN, but tell login to send them to MAIN
                st.session_state.app_mode = "login"
