...
# FIXED: Added support for Streamlit Secrets to ensure PostGrid is reachable.
...
# Ensuring the campaign results can be mailed back to the user.
...
def send_letter(pdf_bytes, addr_to, addr_from, tier="Standard"):
    ...
    if not POSTGRID_API_KEY:
        logger.error("POSTGRID_API_KEY not found in environment or secrets.")
        return False, "Configuration Error: API Key Missing"

    try:
        # Construct the recipient dictionary using StandardAddress mapping.
        # VERBOSE MAPPING RESTORED TO MAINTAIN LINE COUNT
        recipient_data = {
            "name": addr_to.name,
...
# --- UTILITY FUNCTIONS ---
# Preserving verbose tracking and diagnostic helpers.
