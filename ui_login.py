...
def show_login(login_func, signup_func):
    # CSS FIX: Remove default top padding and adjust container margins
    st.markdown("""
    <style>
        /* Force remove top padding from the main block container */
        .block-container {
        ...
        .auth-container {
            max-width: 450px;
            margin: 0 auto; /* Center horizontally, 0 top margin */
            padding: 30px 20px;
            background: white;
            border-radius: 12px;
        ...
        .auth-header { color: #1e3c72; font-weight: 700; margin-bottom: 5px; }
        .auth-sub { font-size: 0.9rem; color: #666; margin-bottom: 20px; }
    </style>
    ...
    # Use a container to hold the auth card, ensuring clean separation
    with st.container():
        # --- VIEW 3: UPDATE PASSWORD (WITH TOKEN) ---
        if st.session_state.auth_view == "update_password":
            st.markdown("<div class='auth-container'><h2 class='auth-header'>🔐 Set New Password</h2>", unsafe_allow_html=True)
            st.markdown("<p class='auth-sub'>Enter the code sent to your email.</p>", unsafe_allow_html=True)
        ...
        if st.session_state.auth_view == "forgot":
            st.markdown("<div class='auth-container'><h2 class='auth-header'>↺ Recovery</h2>", unsafe_allow_html=True)
            st.markdown("<p class='auth-sub'>We'll send a code to your email.</p>", unsafe_allow_html=True)
