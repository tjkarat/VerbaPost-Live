...
class StandardAddress:
    ...
    def to_postgrid_payload(self) -> Dict[str, str]:
        """Returns the dictionary expected by PostGrid API."""
        return {
            'name': self.name,
    ...
    def to_pdf_string(self) -> str:
        """Returns the formatted string for PDF generation."""
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'StandardAddress':
        """Robustly parses any mess of keys into a clean object."""
        if not data: data = {}
        
        # Fallback chains
        street = (data.get('street') or data.get('address_line1') or 
                  data.get('line1') or data.get('address') or "")
        
        line2 = (data.get('address_line2') or data.get('street2') or 
                 data.get('apt') or data.get('suite') or "")
