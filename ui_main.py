                ...
                if payment_engine:
                    ...
                    if st.button("Proceed to Payment", type="primary", use_container_width=True):
                        with st.spinner("Connecting to Stripe..."):
                            # Check returns
                            res = payment_engine.create_checkout_session(
                            ...
                            if res and res[0]:
                                st.session_state.stripe_url = res[0]
                                st.rerun()
                            else:
                                st.error("Stripe Connection Failed. Check Secrets.")
                    
                    if st.session_state.get("stripe_url"):
                        url = st.session_state.stripe_url
                        # CSS BOMB: Force White Text using span !important
                        st.markdown(f"""
