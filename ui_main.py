        ...
        with st.container(border=True):
            ...
            if is_free:
                ...
                if st.button(f"Pay ${price} & Start", type="primary"):
                    ...
                    # We include session_id={{CHECKOUT_SESSION_ID}} so Stripe fills it in
                    ...
                    if url: 
                        # --- CSS FIX: Force White Text via Inline Styles ---
                        st.markdown(f"""
...
def show_main_app():
    ...
    # --- 1. STRIPE RETURN HANDLER (Corrected) ---
    if "session_id" in st.query_params:
        st.session_state.app_mode = "workspace"
        st.session_state.payment_complete = True
        # Also restore tier/lang from URL if needed
