...
def get_supabase():
    ...
    try:
        ...
        if "SUPABASE_URL" not in st.secrets:
            print("DEBUG: SUPABASE_URL missing from secrets")
            return None
...
def render_store_page():
    ...
    with c1:
        with st.container(border=True):
            ...
            # Helper to map display string back to key
        ...
        with st.container(border=True):
            ...
            if is_free:
                ...
                if st.button("Pay & Start", type="primary"):
                    ...
                    if url:
                        st.markdown(f"""<a href="{url}" target="_blank" style="text-decoration:none;"><div style="background-color:#2a5298;color:white;padding:10px;text-align:center;border-radius:5px;">👉 Pay Now (Secure)</div></a>""", unsafe_allow_html=True)
...
def show_main_app():
    ...
    if "session_id" in st.query_params:
        # ... (Logic to verify payment and set mode='workspace') ...
        st.session_state.app_mode = "workspace"
        st.session_state.payment_complete = True
        st.query_params.clear()
        st.rerun()
