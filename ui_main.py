...
def render_hero(title, subtitle):
    # GLOBAL CSS FIX FOR BUTTONS AND TEXT
    st.markdown(f"""
    <style>
...
def render_workspace_page():
    ...
    with c_mic:
        ...
        # AUTO-SAVE ON RECORD IF NOT SAVED
        if not st.session_state.get("to_addr"):
             # Try to auto-populate session state from current inputs before recording
             if is_santa:
                 st.session_state.to_addr = {"name": to_name, "street": to_street, "city": to_city, "state": to_state, "zip": to_zip}
                 st.session_state.from_addr = {"name": "Santa Claus", "street": "123 Elf Road", "city": "North Pole", "state": "NP", "zip": "88888"}
             else:
...
def render_review_page():
    ...
    if st.button("🚀 Send Letter", type="primary"):
        ...
        if not to_a.get("name"): st.error("Recipient Name Missing! Please go back and save address."); return
