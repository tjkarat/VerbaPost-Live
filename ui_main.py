...
import time
import logging
import hashlib # Added for idempotency
...
def _render_hero(title, subtitle):
    """Renders the standard blue gradient header with Dark Mode fix."""
    st.markdown(f"""
    <div class="custom-hero" style="background: linear-gradient(135deg, #1e3c72 0%, #2a5298 100%); padding: 30px 20px; border-radius: 15px; text-align: center; margin-bottom: 25px; box-shadow: 0 8px 16px rgba(0,0,0,0.1); max-width: 100%; box-sizing: border-box;">
        <h1 style="margin: 0; font-size: clamp(1.8rem, 5vw, 3rem); font-weight: 700; color: #FFFFFF !important; line-height: 1.1; text-shadow: 0px 2px 4px rgba(0,0,0,0.5);">{title}</h1>
...
def _render_address_book_selector(u_email):
    """Renders the Quick Fill dropdown."""
...
def _process_sending_logic(tier):
    ...
    if tier == "Campaign": 
        ...
        # FIX: Ensure we use the full address object returned by civic_engine
...
def render_store_page():
    # Pass global URL to ensure visibility
    global YOUR_APP_URL
                        ...
                        try:
                            ...
                            if payment_engine:
                                ...
                                if url:
                                    ...
                                    st.error("⚠️ Stripe Error: Could not generate link.")
                            else:
...
def render_review_page():
    ...
    # --- FIX: Display Civic Targets to User ---
    if tier == "Civic" and st.session_state.get("civic_targets"):
        st.info("🏛️ This letter will be mailed to:")
