...
def reset_app(full_logout=False):
    ...
    """
    # 1. Clear URL immediately
    st.query_params.clear()
    
    # 2. Preserve User
    u_email = st.session_state.get("user_email")
    
    # 3. Nuke Order-Specific Keys
    keys_to_clear = [
...
def _save_addresses_to_state(tier):
    ...
    if tier == "Santa": 
        ...
        st.session_state.from_addr = {"name": st.session_state.get("w_from_name"), "street": st.session_state.get("w_from_street"), "address_line2": st.session_state.get("w_from_street2"), "city": st.session_state.get("w_from_city"), "state": st.session_state.get("w_from_state"), "zip": st.session_state.get("w_from_zip"), "country": "US", "email": u}
    if tier == "Civic":
...
def render_store_page():
    ...
    with c1:
        with st.container(border=True):
            ...
            tier_code = sel
            st.info("Description updated per selection.")
    ...
    with st.container(border=True):
        if tier == "Campaign":
            ...
            with st.form("addressing_form"):
                ...
                if st.form_submit_button("Save Addresses"): _save_addresses_to_state(tier); st.toast("Saved!")
...
def render_review_page():
    ...
    def _ai_fix(style):
        if ai_engine:
            with st.spinner("Rewriting..."): st.session_state.transcribed_text = ai_engine.refine_text(current_text, style); st.rerun()
    ...
    if not current_text: st.warning("Please enter some text before generating a preview.")
        ...
        try:
            ...
            if sig_path: try: os.remove(sig_path); except: pass
    ...
    if st.button("🚀 Send Letter", type="primary"):
        ...
        if tier != "Campaign" and tier != "Civic":
            if not to_check.get("city") or not to_check.get("zip"): st.error("❌ Recipient Address Incomplete!"); return
            ...
            for tgt in targets:
                ...
                if mailer:
                    ...
                    try:
                        ...
                        if os.path.exists(tpath): try: os.remove(tpath); except: pass
