...
def create_pdf(body_text, to_addr, from_addr, tier="Standard", signature_text="", audio_url=None, metadata=None):
    ...
    try:
        ...
        if tier == "Heirloom":
            # --- LAYOUT A: HEIRLOOM (Updated: Removed Exported Date) ---
            ...
            # Line 1: Storyteller Name (Left)
            pdf.set_font(font_family, 'B', 11)
            pdf.cell(0, 5, f"Storyteller: {storyteller}", ln=1)
            
            # Line 2: Recorded Date (Left)
            if interview_date and interview_date != "Undated":
