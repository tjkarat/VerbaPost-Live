...
- PROD (and staging) Supabase → Authentication → SMTP: configure custom SMTP via Resend so auth emails (confirm/reset) send from @verbapost.com instead of Supabase's rate-limited, spam-prone default sender.
...
**Key finding (CORRECTED Jul 6):** the Cloud Run domain mappings for verbapost.com/www are vestigial — DNS for the apex actually points at **Netlify** (75.2.60.5), which serves the marketing site from the separate `tjkarat/verbapost-marketing` repo (auto-deploy on push to main; webhook showed rot once — manual "Trigger deploy" fixes it). **Decision: marketing stays on Netlify permanently** (isolation from app outages, CDN speed, 3-second deploys). Cutover therefore touches ONLY app.verbapost.com traffic — no host routing shim needed in FastAPI, no DNS changes anywhere. Do not delete the vestigial mappings (no benefit, small risk).
