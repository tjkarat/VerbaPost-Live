...
st.markdown(f"""
    ...
    /* Universal Selector for internals */
    a[data-testid="stLinkButton"] * {{
        color: #FFFFFF !important;
...
def check_is_admin():
    ...
    email = u.get("email") if isinstance(u, dict) else (u.email if hasattr(u, "email") else "")
    return email.strip().lower() == t.strip().lower()

with st.sidebar:
    if st.button("🏠 Home", use_container_width=True):
        st.session_state.current_view = "splash"; st.rerun()
...
if v == "splash": ui_splash.show_splash()
    ...
    def signup(e,p,n,s,c,stt,z,l):
        u, err = auth_engine.sign_up(e,p,n,s,c,stt,z,l)
        if u: st.session_state.user=u; st.session_state.current_view="main_app"; st.rerun()
        else: st.error(err)
    ui_login.show_login(login, signup)
